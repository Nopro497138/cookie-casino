...
function PlayingCard({card,hidden,delay=0}:{card:Card,hidden?:boolean,delay?:number}) {
  return (
    <motion.div initial={{x:-60,rotate:-8,opacity:0}} animate={{x:0,rotate:0,opacity:1}} transition={{delay,duration:0.3,type:"spring"}}
      className="card-playing select-none shrink-0"
      style={hidden
        ?{background:"linear-gradient(135deg,#312e81,#1e1b4b)",color:"transparent"}
        :{background:"linear-gradient(135deg,#1e293b,#0f172a)",color:SUIT_COLOR[card.suit]??"#e2e8f0",border:"1px solid rgba(255,255,255,0.15)"}}>
      {!hidden && <>
        <div className="text-xs leading-none font-bold">{card.value}</div>
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-bold px-2 py-0.5 rounded-full"
          style={{background:bust?"rgba(239,68,68,0.2)":score===21?"rgba(245,158,11,0.2)":"rgba(139,92,246,0.15)",color:bust?"#f87171":score===21?"#fbbf24":"#a78bfa"}}>
          {bust?"BUST":score===21&&cards.length===2?"BLACKJACK!":score}
        </span>
      </div>
...
export function Blackjack({balance,onBalanceChange}:{balance:number,onBalanceChange:(n:number)=>void}) {
  ...
  const post = async (body: object) => {
    const r = await fetch("/api/games/blackjack",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});
    return r.json();
  };
  ...
  const deal = async () => {
    ...
    setPScore(d.playerScore); setDScore(d.dealerScore||0);
    setDeck(d.deck||[]);
  ...
  return (
    <div className="h-full flex flex-col gap-4 p-2">
      {/* Table */}
      <div className="flex-1 rounded-2xl p-4 md:p-6 relative overflow-hidden" style={{background:"linear-gradient(135deg,rgba(2,44,34,0.8),rgba(5,78,58,0.4))",border:"1px solid rgba(16,185,129,0.2)"}}>
        <div className="absolute inset-0 rounded-2xl" style={{boxShadow:"inset 0 0 60px rgba(16,185,129,0.05)"}} />

        {phase==="betting" && (
          <div className="flex items-center justify-center h-full">
            <div className="text-center space-y-3">
              <div className="text-5xl">🎴</div>
              <p className="text-slate-400">Place your bet to start</p>
            </div>
          </div>
        )}
        ...
        <AnimatePresence>
          {result && (
            <motion.div initial={{opacity:0,scale:0.8,y:20}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.8}}
              ...
              style={{background:"rgba(0,0,0,0.7)",backdropFilter:"blur(8px)",borderRadius:"1rem"}}>
              <div className="text-center">
                <div className="text-5xl mb-2">{STATUS_MAP[result.status]?.icon}</div>
                <p className="text-2xl font-black mb-1" style={{color:STATUS_MAP[result.status]?.color}}>
      ...
      <div className="glass rounded-xl p-4 space-y-3">
        {phase==="betting" && (
          <>
            ...
            <button onClick={deal} disabled={loading||balance<bet} className="btn-primary w-full py-3">
          ...
          <div className="flex gap-2">
            {[{a:"hit",l:"Hit",e:"👆"},{a:"stand",l:"Stand",e:"✋"},{a:"double",l:"Double",e:"×2"}].map(({a,l,e})=>(
              <button key={a} onClick={()=>action(a)} disabled={loading}
                ...
                style={{background:a==="stand"?"rgba(239,68,68,0.15)":"rgba(139,92,246,0.15)",border:"1px solid",borderColor:a==="stand"?"rgba(239,68,68,0.3)":"rgba(139,92,246,0.3)",color:a==="stand"?"#f87171":"#a78bfa"}}>
                {e} {l}
              </button>
            ))}
          </div>
