...
export function Crash({
  ...
  const startRound = async () => {
    ...
    const res = await fetch("/api/games/simple", {
      ...
      body: JSON.stringify({ game: "crash", bet, cashOutAt: autoCash }),
    });
  ...
  const cashOutNow = async () => {
    ...
    const res = await fetch("/api/games/simple", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ game: "crash", bet, cashOutAt: at }),
    });
  ...
  const multColor =
    mult < 1.5 ? "#10b981" : mult < 2 ? "#f59e0b" : mult < 5 ? "#f97316" : "#ef4444";

  return (
    <div className="h-full flex flex-col gap-4 p-2">
      <div
        className="flex-1 rounded-2xl flex flex-col items-center justify-center gap-4 p-6 relative overflow-hidden"
        style={{
          background: "linear-gradient(180deg,rgba(8,8,15,0.9),rgba(30,8,8,0.6))",
          border: `1px solid ${
            phase === "crashed" ? "rgba(239,68,68,0.4)" : "rgba(139,92,246,0.2)"
          }`,
        }}
      >
        {/* Rocket */}
        {phase === "flying" && (
          <motion.div
            ...
            className="text-5xl absolute top-6 right-8"
        ...
        <motion.div
          ...
          style={{
            color: phase === "crashed" && !cashedOut ? "#ef4444" : cashedOut ? "#10b981" : multColor,
            textShadow: `0 0 40px ${
        ...
        <p className="text-slate-500 text-sm">
          {phase === "idle"
            ? "Set your auto cash-out and start"
            : phase === "flying"
        ...
        {result && (
          <motion.div
            ...
            style={{
              background: result.won ? "rgba(16,185,129,0.15)" : "rgba(239,68,68,0.1)",
              border: `1px solid ${
                result.won ? "rgba(16,185,129,0.4)" : "rgba(239,68,68,0.3)"
              }`,
            }}
          >
            {result.won ? (
              <p className="text-green-400 font-bold text-lg">
                ✅ Cashed out at {result.at}× — +🍪{fmt(result.net)}
              </p>
            ) : (
              <p className="text-red-400 font-bold text-lg">
                💥 Crashed at {result.at}× — -🍪{fmt(bet)}
              </p>
            )}
          </motion.div>
        )}
      ...
      <div className="glass rounded-xl p-4 space-y-3">
        {phase === "idle" || phase === "crashed" ? (
          <>
            <BetInput bet={bet} onChange={setBet} balance={balance} disabled={isFlying} />
