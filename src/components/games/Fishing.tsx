  ...
  return (
    <div className="h-full flex flex-col gap-4 p-2">
      <div className="flex-1 rounded-2xl flex flex-col items-center justify-center relative overflow-hidden"
        ...
        {/* Water animation */}
        <div className="absolute bottom-0 left-0 right-0 h-24 opacity-30" style={{background:"linear-gradient(0deg,rgba(59,130,246,0.4),transparent)"}} />

        {/* Fish swimming */}
        {(phase==="waiting"||phase==="idle") && (
          <>
            {["🐟","🐠","🐡"].map((f,i)=>(
              <motion.div key={i} animate={{x:["-10%","110%"],y:[0,Math.sin(i)*20,0]}}
                transition={{duration:5+i*2,delay:i*1.5,repeat:Infinity,ease:"linear"}}
                className="absolute text-xl opacity-40" style={{bottom:`${20+i*12}%`}}>{f}</motion.div>
            ))}
          </>
        )}
        ...
        {(phase==="casting"||phase==="waiting"||phase==="reeling") && (
          <div className="absolute top-8 flex flex-col items-center">
            ...
            <motion.div animate={{height:phase==="casting"?[0,120]:phase==="reeling"?[120,0]:[120]}}
              ...
              className="w-0.5 origin-top" style={{background:"rgba(255,255,255,0.3)"}} />
            {(phase==="waiting"||phase==="reeling") && (
              <motion.div animate={{y:phase==="waiting"?[0,6,0]:undefined}} transition={{duration:0.8,repeat:phase==="waiting"?Infinity:0}}
        ...
        <AnimatePresence>
          {phase==="done" && catch_ && (
            <motion.div initial={{scale:0,rotate:-10}} animate={{scale:1,rotate:0}} exit={{scale:0}}
              ...
              style={{background:"rgba(0,0,0,0.7)",backdropFilter:"blur(12px)",border:`2px solid ${color}`,boxShadow:`0 0 30px ${color}44`}}>
              <motion.div animate={{rotate:[0,10,-10,0]}} transition={{duration:0.5,delay:0.2}} className="text-6xl">
                {catch_.emoji}
              </motion.div>
              <div className="text-center">
                <p className="text-xs uppercase tracking-widest font-bold mb-1" style={{color}}>{catch_.rarity}</p>
                <p className="text-xl font-bold text-white">{catch_.name}</p>
                {catch_.value > 0 ? (
                  <p className="text-lg font-black mt-1" style={{color:"#10b981"}}>+🍪{fmt(net > 0 ? net : 0)} profit</p>
                ) : (
      ...
      <div className="glass rounded-xl p-4 space-y-3">
        ...
        <button onClick={phase==="done"?()=>setPhase("idle"):cast}
          disabled={(phase!=="idle"&&phase!=="done")||balance<bet}
